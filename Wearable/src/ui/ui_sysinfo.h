...
struct ElementSysInfoT : public ElementT {
    ...
    void on_draw(Event *event) override {
        ...
        writer.printf(Sprites::minifont, "ALS %li %.01flx\n", als, ltr390.getLux(als));
        writer.printf(Sprites::minifont, "OLED %.3fV %.3fmA\n", Sprites::display.getVoltageCOM(), Sprites::display.getDisplayCurrentDraw()*1000.0f);
        writer.printf(Sprites::minifont, "OLED %.1f%% %.1fHz\n", float(Sprites::display.display_contrast)/255.0f*100.0f, Sprites::display.getDisplayRefreshFrequency());
        writer.printf(Sprites::minifont, "OLED pre %i dis %i\n", Sprites::display.precharge_period, Sprites::display.discharge_period);
        writer.printf(Sprites::minifont, "Free Heap ");
        writer.template print_value<ValueBases::SIByte>(Sprites::minifont, esp_get_free_heap_size());
        writer.printf(Sprites::minifont, "\nSD Free ");
        writer.template print_value<ValueBases::SIByte>(Sprites::minifont, sdcard.getFreeSize());
        writer.printf(Sprites::minifont, "\nSD Size ");
        writer.template print_value<ValueBases::SIByte>(Sprites::minifont, sdcard.getTotalSize());
        writer.printf(Sprites::minifont, "\nCPU %.1fMHz %.1fMHz\n", float(wbl_system.getCPUFreq())*0.000001, float(wbl_system.testCPUFreq())*0.000001);
    }
};
